...
if __name__ == "__main__":
  ...
  for fname in sys.argv[1:]:
    ...
    im1 = ax1.imshow(I, vmax=vmax, vmin=vmin,cmap='afmhot', origin='lower', extent=extent)
    if not do_fullscreen_unpol:
      colorbar(im1)

    if not no_pol:
      # linear polarization fraction
      Q_vmin = Q.min()#/np.sqrt(1.5)
      Q_vmax = Q.max()#/np.sqrt(1.5)
      ax2.set_facecolor('black')
      im2 = ax2.imshow(Q, cmap='seismic', vmin=Q.min(), vmax=Q.max(), origin='lower', extent=extent)
      colorbar(im2)
      ...
      im4 = ax4.imshow(U, cmap='seismic', vmin=U.min(), vmax=U.max(), origin='lower', extent=extent)
      colorbar(im4)
      ...
      im3 = ax3.imshow(V, cmap='seismic', vmin=V.min(), vmax=V.max(), origin='lower', extent=extent)
      colorbar(im3)
